...
for i in range(Parm.task_number):
    ...
    task.train = Data.TensorDataset(torch.stack(data[0][0]).type(torch.FloatTensor)[:7000], 
                                    torch.tensor(data[0][1]).type(torch.LongTensor)[:7000])
...
def Tasks_initial(Tasks, Parm, hook=True):
    for i in range(Parm.task_number):
        Tasks[i].model = copy.deepcopy(Plugin(Tasks[i].model0, cpu=False))
...
# record.record('./result/e2_2C', Parm, 'pkl')